...
const legacyCoreSitemapPattern = /^sitemap(\.xml|-\d+\.xml)$/i;
...
async function removeLegacyCoreSitemaps() {
  try {
    const files = await readdir(outDir);
    const legacy = files.filter((file) => legacyCoreSitemapPattern.test(file));
...
async function main() {
  ...
  await removeLegacyCoreSitemaps();

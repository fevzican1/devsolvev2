...
const staleSitemapPattern = /^sitemap-main-pages(\.xml|-\d+\.xml)$/i;
...
async function removeStaleRenamedCoreSitemaps() {
  try {
    const files = await readdir(outDir);
    const stale = files.filter((file) => staleSitemapPattern.test(file));
    await Promise.all(stale.map((file) => rm(join(outDir, file), { force: true })));
  } catch {
    // Ignore when directory is not present yet.
  }
...
async function writeSitemapIndex(programmaticSitemaps, lastmod) {
  const sitemapEntries = programmaticSitemaps;
...
async function main() {
  ...
  await removeStaleRenamedCoreSitemaps();
  ...
  // sitemap.xml (core pages) is generated by next-sitemap and kept out of this index
  // so Google does not see the same file as both a root sitemap and a child entry.
  await writeSitemapIndex(generatedFiles, lastmod);

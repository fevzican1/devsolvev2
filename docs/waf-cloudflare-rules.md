...
Google/Bing skip on User-Agent. Chrome renderers skip only from Google crawler
ASNs or unstamped Chrome on Bing/Microsoft ASNs. **No chrome-extension in
WAF1** — WAF2 owns that. Public files (`robots.txt`, sitemap, …) skip only
when the UA is not Semrush/Ahrefs/farm `.0.0.0`. **Applebot is not listed.**

```
(lower(http.user_agent) contains "google" or lower(http.user_agent) contains "bing" or lower(http.user_agent) contains "msn" or lower(http.user_agent) contains "adidx" or lower(http.user_agent) contains "microsoftpreview" or lower(http.user_agent) contains "twitter" or lower(http.user_agent) contains "facebook" or lower(http.user_agent) contains "facebot" or lower(http.user_agent) contains "linkedin" or lower(http.user_agent) contains "slack" or lower(http.user_agent) contains "discord" or lower(http.user_agent) contains "whatsapp" or lower(http.user_agent) contains "telegram" or lower(http.user_agent) contains "reddit" or lower(http.user_agent) contains "pinterest" or (lower(http.user_agent) contains "chrome/" and (ip.src.asnum in {15169 19527 36040 36561 43515 139070 139190} or (ip.src.asnum in {3598 6182 8068 8069 8070 8071 8072 8075 11271 12076 23274} and not lower(http.user_agent) contains ".0.0.0" and not lower(http.user_agent) contains "chrome/100.0.4896" and not (lower(http.user_agent) contains "mac os x 10_15_7" and lower(http.user_agent) contains "chrome/")))) or (http.request.uri.path in {"/robots.txt" "/sitemap.xml" "/feed.xml" "/opengraph-image.png" "/ee5098cac2284d92b6ee1c9fca52a120.txt" "/ads.txt" "/sellers.json"} and not lower(http.user_agent) contains ".0.0.0" and not lower(http.user_agent) contains "chrome/100.0.4896" and not (lower(http.user_agent) contains "mac os x 10_15_7" and lower(http.user_agent) contains "chrome/") and not lower(http.user_agent) contains "semrush" and not lower(http.user_agent) contains "ahrefs" and not lower(http.user_agent) contains "gpt" and not lower(http.user_agent) contains "oai-" and not lower(http.user_agent) contains "claude" and not lower(http.user_agent) contains "anthropic" and not lower(http.user_agent) contains "perplexity" and not lower(http.user_agent) contains "bytespider" and not lower(http.user_agent) contains "ccbot" and not lower(http.user_agent) contains "dataforseo" and not lower(http.user_agent) contains "headless" and not lower(http.user_agent) contains "puppeteer" and not lower(http.user_agent) contains "selenium" and not lower(http.user_agent) contains "curl" and not lower(http.user_agent) contains "wget" and not lower(http.user_agent) contains "python" and not lower(http.user_agent) contains "scrapy"))
```

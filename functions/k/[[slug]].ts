...
function resolvePageFromSlug(slug: string): PageData | undefined {
  ...
  const title = titleTemplate
    .replace('{intent}', slugToWords(pair.intent))
    .replace('{audience}', slugToWords(audience))
  ...
  const h1 = h1Template
    .replace('{intent}', slugToWords(pair.intent))
    .replace(/\{audience\}/g, slugToWords(audience));

  // Build description
  const descVariants = [
    `${title} — practical, browser-based workflow for real-world ${slugToWords(clusterKey)} engineering tasks, ${slugToWords(modifier)}. Learn how to ${tc.outcome} with ${toolName}.`,
    `Step-by-step guide to ${slugToWords(pair.intent)} using ${toolName} for ${slugToWords(audience)} professionals. Covers ${tc.scenario} with best practices for ${cd.field}.`,
    `How ${slugToWords(audience)} teams use ${toolName} to ${slugToWords(pair.intent)} ${slugToWords(modifier)}. Includes troubleshooting tips, alternative solutions, and expert recommendations.`,
    `Complete walkthrough: ${slugToWords(pair.intent)} with ${toolName} for ${slugToWords(audience)} workflows. All processing runs locally in your browser — your data stays private.`,
    `A ${slugToWords(audience)}'s guide to ${slugToWords(pair.intent)} using browser-based ${toolName}. Practical steps for ${tc.scenario}, with focus on ${ac.focus}.`,
  ];
  const description = descVariants[seed % descVariants.length];

  // Build intro
  const introVariants = [
    `As a ${slugToWords(audience)} focused on ${ac.focus}, you can ${slugToWords(pair.intent)} using the browser-based ${toolName}. ${cd.importance}, and this guide walks through the process ${slugToWords(modifier)}. The scenario here is ${tc.scenario}, which is ${tc.urgency}. By the end, you will ${tc.outcome} — all without sending data to an external server.`,
    `This page explains how a ${slugToWords(audience)} can approach ${slugToWords(pair.intent)} with ${toolName}, ${slugToWords(modifier)}. In the context of ${cd.field}, ${cd.importance.toLowerCase()}. The specific focus is on ${tc.scenario}, and the goal is to ${tc.outcome}. Every step runs locally in your browser, so your data stays private — an important consideration given ${ac.concern}.`,
    `When ${tc.scenario}, a ${slugToWords(audience)} needs reliable tools for ${slugToWords(pair.intent)}. ${toolName} handles this ${slugToWords(modifier)}, with all processing happening locally in your browser. This is particularly relevant because ${cd.importance.toLowerCase()}. The workflow is designed ${ac.workflow}, with the goal to ${tc.outcome}.`,
    `For ${slugToWords(audience)} professionals working on ${cd.field}, ${slugToWords(pair.intent)} is a common requirement. This guide shows how to accomplish this using ${toolName} ${slugToWords(modifier)}. The real-world context is ${tc.scenario} — ${tc.urgency}. ${cd.bestPractice}. All processing runs locally, addressing ${ac.concern}.`,
    `${slugToWords(pair.intent)} is a task that every ${slugToWords(audience)} encounters in ${cd.field}. Using ${toolName} ${slugToWords(modifier)}, you can handle this efficiently and securely. This walkthrough targets ${tc.scenario}, helping you ${tc.outcome}. The browser-based approach means your data never leaves your machine, which matters when dealing with ${ac.concern}.`,
  ];
  ...
  const steps = [
    ...
    `Paste or type your input for the ${slugToWords(pair.intent)} operation. If working with sensitive data, verify that your browser environment is secure.`,
  ...
  const keywords = [
    ...
    `${slugToWords(pair.intent)} tool`, `${slugToWords(audience)} ${slugToWords(clusterKey)} guide`,
    `browser-based ${slugToWords(pair.tool)}`, 'developer tool', 'free online tool',
  ];
...
function tryResolveLegacyProgrammaticSlug(slug: string): PageData | undefined {
  ...
  // old format only expressed coarse position, not exact modifier identity.
...
function generateHtml(page: PageData): string {
  ...
  const faqItems = [
    { q: `What does ${slugToWords(page.intent)} mean for a ${slugToWords(page.audience)}?`, a: `For a ${slugToWords(page.audience)} focused on ${ac.focus}, ${slugToWords(page.intent)} involves using ${toolName} to ${tc.outcome}. This is done ${slugToWords(page.modifier)} to ensure efficiency and data privacy.` },
    { q: `Is my data safe when using ${toolName}?`, a: `Yes. ${toolName} runs entirely in your browser. No data is transmitted to any external server. This is especially important for ${slugToWords(page.audience)} professionals concerned about ${ac.concern}.` },
    ...
    { q: `What are the best practices for ${slugToWords(page.clusterKey)} tasks?`, a: `${cd.bestPractice}. ${cd.importance}, making proper tooling essential for ${slugToWords(page.audience)} workflows.` },
  ];
...
<p>This page explains step by step how to handle the <strong>${escapeHtml(slugToWords(page.intent))}</strong> task in real project scenarios using <strong>${escapeHtml(toolName)}</strong>.</p>
...
<p>${escapeHtml(cd.importance)}. For ${escapeHtml(slugToWords(page.audience))} professionals, this means paying close attention to ${escapeHtml(ac.focus)}. The best practice is: ${escapeHtml(cd.bestPractice)}.</p>
<p>In the context of ${escapeHtml(tc.scenario)}, which is ${escapeHtml(tc.urgency)}, the objective is to ${escapeHtml(tc.outcome)}. Using the approach described ${escapeHtml(slugToWords(page.modifier))} ensures efficiency without compromising on quality or security.</p>

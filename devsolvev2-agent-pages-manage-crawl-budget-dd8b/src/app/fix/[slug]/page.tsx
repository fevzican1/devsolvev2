...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata({
    title, description, path: `/fix/${slug}`,
    keywords: [error.errorName, `${library.name} ${library.version}`, `${library.name} ${error.errorName}`, language.label, 'error fix', 'how to fix', library.pkg],
    datePublished: siteConfig.launchDate, dateModified: siteConfig.contentUpdatedAt, articleSection: language.label,
  });
}
...
export default async function FixPage({ params }: PageProps) {
  ...
  const verifySteps = (e.verifySteps as string[]) ?? ['Rerun your application to confirm the error no longer appears.', 'Check that the expected output matches the fix above.', 'Monitor logs for any remaining occurrences of the same error pattern.'];
  ...
  const faqJsonLd = { '@context': externalUrls.schemaOrg, '@type': 'FAQPage', mainEntity: [{ '@type': 'Question', name: `What causes ${error.errorName} in ${library.name} ${library.version}?`, acceptedAnswer: { '@type': 'Answer', text: error.cause } }, { '@type': 'Question', name: `How do I fix ${error.errorName} in ${library.name}?`, acceptedAnswer: { '@type': 'Answer', text: error.explanation } }, { '@type': 'Question', name: `How can I prevent ${error.errorName} from happening again?`, acceptedAnswer: { '@type': 'Answer', text: error.prevention } }] };
  const howToJsonLd = { '@context': externalUrls.schemaOrg, '@type': 'HowTo', name: pageTitle, description: pageDescription, step: [{ '@type': 'HowToStep', position: 1, name: 'Reproduce', text: `Reproduce the error: ${error.summary}` }, { '@type': 'HowToStep', position: 2, name: 'Diagnose', text: error.cause }, { '@type': 'HowToStep', position: 3, name: 'Fix', text: error.explanation }, { '@type': 'HowToStep', position: 4, name: 'Prevent', text: error.prevention }] };
  ...
  const breadcrumbJsonLd = { '@context': externalUrls.schemaOrg, '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: absoluteUrl('/') }, { '@type': 'ListItem', position: 2, name: 'Error fixes', item: absoluteUrl('/fix') }, { '@type': 'ListItem', position: 3, name: error.errorName, item: absoluteUrl(`/fix/${slug}`) }] };
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="mb-8"><CardHeader className="pb-3"><CardTitle className="text-base">Fix at a glance</CardTitle><CardDescription className="text-xs">Reviewed by the DevSolve Editorial Team · Last verified {reviewedOn}</CardDescription></CardHeader><CardContent><dl className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4"><div><dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground"><ShieldAlert className="h-3 w-3" /> Severity</dt><dd className={`mt-1 text-sm font-semibold capitalize ${SEVERITY_STYLE[severity] ?? 'text-yellow-600'}`}>{severity}</dd></div><div><dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground"><Gauge className="h-3 w-3" /> Difficulty</dt><dd className="mt-1 text-sm font-semibold">{DIFFICULTY_LABEL[difficulty] ?? 'Intermediate'}</dd></div><div><dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground"><Clock className="h-3 w-3" /> Time to fix</dt><dd className="mt-1 text-sm font-semibold">{timeEstimate}</dd></div><div><dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground"><Layers className="h-3 w-3" /> Applies to</dt><dd className="mt-1 text-sm font-semibold">{affectedVersions}</dd></div></dl><div className="mt-4 flex flex-wrap items-center gap-2 border-t pt-4"><Tags className="h-3.5 w-3.5 text-muted-foreground" />{tags.map((tag) => <Badge key={tag} variant="secondary" className="text-[11px] font-normal">{tag}</Badge>)}</div></CardContent></Card>
        ...
        <Card className="mb-8"><CardHeader><CardTitle className="text-lg flex items-center gap-2"><Bug className="h-5 w-5" />Reproduce it ({language.label})</CardTitle><CardDescription className="text-xs">Minimal {library.name} {library.version} snippet that triggers the error.{codeVerified && <span className="block mt-1 text-green-600 text-[11px]">This code has been tested and verified{verifiedPlatform ? ` on ${verifiedPlatform}` : ''}.</span>}</CardDescription></CardHeader><CardContent className="space-y-3"><pre className="overflow-x-auto rounded-md border bg-muted/40 p-3 text-xs" data-lang={language.codeLang}><code>{error.reproduce}</code></pre><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground mb-1">Why it happens</p><p className="text-sm text-muted-foreground">{error.cause}</p></div></CardContent></Card>
        <Card className="mb-8 border-success/40 bg-success/5"><CardHeader><CardTitle className="text-lg flex items-center gap-2"><CheckCircle className="h-5 w-5 text-success" />The fix ({language.label})</CardTitle><CardDescription className="text-xs">Copy-paste solution for {library.name} {library.version}.</CardDescription></CardHeader><CardContent className="space-y-3"><pre className="overflow-x-auto rounded-md border bg-muted/40 p-3 text-xs" data-lang={language.codeLang}><code>{error.fix}</code></pre><div><p className="text-xs font-medium uppercase tracking-wide text-muted-foreground mb-1">Why the fix works</p><p className="text-sm text-muted-foreground">{error.explanation}</p></div></CardContent></Card>
        <Card className="mb-8 border-primary/25 bg-primary/5"><CardHeader><CardTitle className="text-lg flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-primary" />Prevent it from recurring</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">{error.prevention}</p></CardContent></Card>
        <Card className="mb-8"><CardHeader><CardTitle className="text-lg flex items-center gap-2"><ListChecks className="h-5 w-5" />Verify the fix</CardTitle><CardDescription className="text-xs">Confirm the {error.errorName} is genuinely resolved before you ship.</CardDescription></CardHeader><CardContent><ol className="space-y-2">{verifySteps.map((step, i) => <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[11px] font-semibold text-foreground">{i + 1}</span><span>{step}</span></li>)}</ol></CardContent></Card>
        {relatedTool && <Card className="mb-8"><CardHeader><CardTitle className="text-lg flex items-center gap-2"><Wrench className="h-5 w-5" />Diagnose with a free browser tool</CardTitle></CardHeader><CardContent><Link href={`/tools/${relatedTool.slug}`}><Card className="hover:shadow-sm transition-shadow"><CardHeader className="py-3 px-4"><div className="flex items-center justify-between"><CardTitle className="text-sm font-medium">{relatedTool.name}</CardTitle><ArrowRight className="h-4 w-4 text-muted-foreground" /></div><CardDescription className="text-xs">{relatedTool.shortDescription}</CardDescription></CardHeader></Card></Link></CardContent></Card>}

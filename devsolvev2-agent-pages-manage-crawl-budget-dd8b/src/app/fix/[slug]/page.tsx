...
export default async function FixPage({ params }: PageProps) {
  ...
  const severity: string = (e.severity as string) ?? 'medium';
  const difficulty: string = (e.difficulty as string) ?? 'intermediate';
  const timeEstimate: string = (e.timeEstimate as string) ?? '~15 min';
  const affectedVersions: string = (e.affectedVersions as string) ?? 'current';
  const tags: string[] = (e.tags as string[]) ?? [];
  const verifySteps: string[] = (e.verifySteps as string[]) ?? [
  ...
  const codeVerified: boolean = (e.codeVerified as boolean) ?? false;
  const verifiedPlatform: string = (e.verifiedPlatform as string) ?? '';
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {codeVerified && (
            <Badge variant="default" className="bg-green-600/10 text-green-700 hover:bg-green-600/20 flex items-center gap-1">
              <Verified className="h-3 w-3" />
              Code verified{verifiedPlatform ? ` on ${verifiedPlatform}` : ''}
            </Badge>
          )}
        </div>
          ...
          <CardContent>
            <dl className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4">
              <div>
                ...
                <dd className={`mt-1 text-sm font-semibold capitalize ${SEVERITY_STYLE[severity] ?? 'text-yellow-600'}`}>
                  {severity}
                </dd>
              </div>
              <div>
                ...
                <dd className="mt-1 text-sm font-semibold">{DIFFICULTY_LABEL[difficulty] ?? 'Intermediate'}</dd>
              </div>
              <div>
                <dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  <Clock className="h-3 w-3" /> Time to fix
                </dt>
                <dd className="mt-1 text-sm font-semibold">{timeEstimate}</dd>
              </div>
              <div>
                <dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  <Layers className="h-3 w-3" /> Applies to
                </dt>
                <dd className="mt-1 text-sm font-semibold">{affectedVersions}</dd>
              </div>
            </dl>
        ...
        <Card className="mb-8 border-muted">
          <CardContent className="py-4">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium">Was this fix helpful?</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {library.version} on {language.ecosystem}{codeVerified ? ', with code tested on a real environment' : ''}.
                Last updated {reviewedOn}. Every code sample runs locally — {siteConfig.name} never asks for your source code, credentials, or data.
              </p>
            </div>
          </div>
        </div>
